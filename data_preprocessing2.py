# -*- coding: utf-8 -*-
"""
Created on Sun Oct 27 11:18:05 2019
...
combine = combine.rename(columns={"ANNUAL":"ANNUAL_RAINFALL"})
combine = pd.concat([combine,repeat],axis=1)
combine = combine[["SUBDIVISION","YEAR","ANNUAL_TEMP","ANNUAL_RAINFALL","Dengue cases"]]
...
for i in range(0,len(combine),117):
    df_new = combine.iloc[i+99:i+110,:]
    name = dir_path+ "\\Preprocessed_micro_Datasets\\" +  df_new.iloc[0,0]+".csv"
    print(name)
